...
namespace Adventofcode2024.Tests.UnitTests.Day3
{
    /*
     --- Day 3: Mull It Over ---
    ...
    public class MultiplyByValidNumbersTest : CorruptedFileTest 
    {
        [Fact]
    ...
    public class MultiplyMulDataTest : CorruptedFileTest
    {
        [Fact]
        public async void Should_Return_161() 
        { 
            var filereader = mockFileReader.CreateMockFileReaderHelper();

            var result = await MultiplyMulData(filereader);

            result.Should().Be(161);
        }

        private async Task<int> MultiplyMulData(FileReaderHelper fileReaderHelper)
        {
